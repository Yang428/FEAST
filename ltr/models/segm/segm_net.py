...
class invol(nn.Module):
    ...
    def __init__(self, input_dim=64, ksz=3, stride=1, dilation=2, padding=2, group=8):
        super().__init__()
        self.ksz = ksz
        self.group = group
        self.span = conv(input_dim, ksz*ksz*group, kernel_size=1, padding=0)
        self.unfold = nn.Unfold(ksz, dilation=dilation, padding=padding, stride=stride)

    def forward(self, f):
        bsz, chl, wh, gsz, ksz = f.size()[0], f.size()[1], f.size()[2], self.group, self.ksz
        f_unfolded = self.unfold(f).permute(0,2,1).contiguous().view(bsz, gsz, chl//gsz, ksz*ksz, wh, wh) #[B,G,C//G,KxK,H,W]
        kernel = self.span(f).view(bsz, gsz, ksz*ksz, wh, wh).unsqueeze(2) # kernel generation [B,G,KxK,1,H,W]
        out = torch.matmul(kernel, f_unfolded).sum(dim=3).view(bsz, chl, wh, wh)
...
class SegmNet(nn.Module):
    def __init__(self, segm_input_dim=(128,256), segm_inter_dim=(256,256), segm_dim=(64, 64), mixer_channels=2, topk_pos=3, topk_neg=3):
        ...
        self.pat_conv0 = conv(segm_input_dim[3], segm_inter_dim[3], kernel_size=1, padding=0) ## 1x1 conv, 3x3 invo for patch net
        self.pat_conv1 = invol(input_dim=64, ksz=3, stride=1, dilation=2, padding=2, group=8)
        self.pat_conv2 = conv(segm_inter_dim[3], segm_inter_dim[3])
        self.pat_conv3 = conv_no_relu(segm_inter_dim[3], segm_inter_dim[3])

        self.mixer0 = conv(mixer_channels, segm_inter_dim[2])    ## mix features
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None, feat_ups=None, up_masks=None, segm_update_flag=False):
        ...
        f_att = self.att_conv3(self.att_conv2(self.fusion(self.att_conv1(self.att_conv0(feat_train[3])), self.att_conv1(self.att_conv0(feat_test[3]))))) 
        f_patch = self.pat_conv1(self.pat_conv0(feat_test[3]))  
        out = self.s3_1(self.s3_0(F.interpolate(f_att + self.pat_conv3(self.pat_conv2(out + f_patch)), scale_factor=2, mode='bilinear', align_corners=False)))
