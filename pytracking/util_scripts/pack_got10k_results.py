...
################### Changed by Yang 2020.06.19 ####################
## txt文件内容为字符型，不能直接读取为float64; 测试结果一次 ##
        for run_id in range(1):
            res = np.loadtxt('{}/{}/{}_{:03d}/{}.txt'.format(results_path, tracker_name, param_name, run_id, seq_name), dtype=np.float64)
            #res = np.loadtxt('{}/{}/{}_{:03d}/{}.txt'.format(results_path, tracker_name, param_name, run_id, seq_name), dtype=np.str)
            #temp = res.tolist()
